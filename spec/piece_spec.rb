...
describe Piece do
  ...
  let(:black_queen) { double('queen', class: Queen, color: 'black') }
  ...
  describe '.intermediate_spots_open?' do
    context 'a spot is filled with a piece of the same color' do
      it 'returns false' do 
        path = [[3, 2], [4, 2], [5, 2], [6, 2]]
        allow(board).to receive(:get_piece).and_return(nil, nil, white_king, nil)
    ...
    context 'a spot is filled with a piece of a different color' do
      it 'returns false' do 
        path = [[3, 2], [4, 2], [5, 2], [6, 2]]
        allow(board).to receive(:get_piece).and_return(nil, black_queen, nil, nil)
        expect(subject.intermediate_spots_open?(board, path)).to be false
      end
    end
